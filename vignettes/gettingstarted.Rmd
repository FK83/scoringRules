...
The following piece of code computes the \textsf{crps} for this situation:
```{r}
library(scoringRules)
# CRPS of a normal distribution with mean = standard deviation = 2, outcome is zero
crps(y = 0, family = "normal", mean = 2, sd = 2)
```
As documented under `?crps`, the parametric family used in the example is one of 15 choices currently implemented in **scoringRules**: 

- Laplace, Logistic, Normal, Mixture of Normals, Two-piece-normal, (Scaled) Student t, Exponential, Gamma, Log-Laplace, Log-Logistic, Log-Normal, Generalized Pareto, Generalized Extreme Value, Uniform, Beta

Whenever possible, our syntax and parametrization closely follow base R. For distributions not supported by base R, we have created documentation pages with details; see for example `?f2pnorm`.
...
#plot(h, xlab = "Value", ylab = "Probability", main = "", 
#     ylim = c(0, max(c(h$counts, n_approx))))
#lines(x = grid, y = n_approx)
