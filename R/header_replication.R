...
#' @details The full results in Section 5 of KLTG (2020) are based on the following setup: \code{burnin_size = 10000},
...
#' @references Krueger, F., Lerch, S., Thorarinsdottir, T.L. and T. Gneiting (2020): `Predictive inference based on Markov chain Monte Carlo output', \emph{International Statistical Review}, forthcoming. \doi{10.1111/insr.12405}
...
#' @details The plot is in the same format as Figure 5 in KLTG (2020). Its content (nr of MCMC chains, 
...
#' @param s,a,n parameters characterizing the process from which data are simulated (see Section 4 and Table 4 of KLTG, 2019). Defaults to the values reported in the main text of the paper.
#' @param nr_iterations number of Monte Carlo iterations (defaults to 50). 
#' @param zoom set to \code{TRUE} to produce results for a fine grid of small (MCMC) sample sizes, as in Figure 2 of KLTG (2020).
...
#' @details The full results in Section 4 of KLTG (2020) are based on \code{s = 2}, \code{a = 0.5},
...
#' @param ... additional parameters, see details below. 
#' @return none, used for the effect of drawing a plot. 
#' @details The plot is in the same format as Figure 1 or 2 in KLTG (2020), depending on the 
...
#' @param Hm1_delta,mu_delta,s_,nu_,R prior parameters as described in KLTG (2020, Appendix E and Table 4).
...
#' }
#' @details The default parameters are as set by KLTG (2020, Section 5). The output matrices \code{fcMeans} and \code{fcSds} can be used to construct 
...
#' @seealso \link{run_casestudy} uses \link{ar_ms} to replicate the results of KLTG (2020, Section 5).
